 ...
 * @since version 1.0
 */
public class LastMatchingOrLessThanIntBinarySearchTest {
    private int[] array;
    ...
    @Test
    public void test() {
        IntBinarySearch binarySearch = new LastMatchingOrLessThanIntBinarySearch();
        assertThat(binarySearch.search(this.array, this.greaterThanAll)).isEqualTo(this.array.length - 1);
        assertThat(binarySearch.search(this.array, this.lessThanAll)).isEqualTo(-1);
        int position = binarySearch.search(this.array, this.keyExists);
        assertThat(position).isGreaterThan(-1);
        if (position > 0) {
            assertThat(this.array[position - 1]).isLessThanOrEqualTo(this.array[position]);
        }
        ...
        assertThat(position).isGreaterThan(-1);
        assertThat(this.array[position]).isLessThan(this.halfBound);
        assertThat(this.array[position + 1]).isGreaterThan(this.halfBound);
    }
}
